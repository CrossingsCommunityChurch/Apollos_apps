...
export default class Schedule extends RockApolloDataSource {
  ...
  getFromIds = (ids) => Promise.all(ids.map((id) => this.getFromId(id)));
    ...
    if (id) {
      ...
      if (schedule) {
        ...
        return filteredOccurrences
          .map((o) => ({
            ...o,
            startWithOffset: moment(o.start)
              .subtract(startOffset, 'm')
              .toISOString(),
          }))
  ...
  async _parseNamedSchedule(id) {
    ...
    const scheduleEndOffsetMinutes = toNumber(
      get(jsonResponse, 'endOffsetMinutes', 0)
    );
  ...
  _parseWeeklySchedule({ weeklyDayOfWeek, weeklyTimeOfDay }) {
    ...
    const endOfMeetingDay = moment(nextStart)
      .endOf('day')
      .utc()
      .format();
    const isAfter = moment().isAfter(endOfMeetingDay);
    if (isAfter) {
      const newNextStart = moment(nextStart)
        .add(7, 'd')
        .utc()
        .format();
